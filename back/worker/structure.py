...
Провайдер задаётся конфигурацией: нужен любой OpenAI-совместимый эндпоинт, и
клиент здесь один и тот же независимо от того, чей он.
...
def structure(descriptions: list[str]) -> tuple[list[dict], dict]:
    ...
    #
    # Но и жадничать нельзя. Модель стадии — рассуждающая (gpt-oss на Groq), и
    # токены рассуждения входят в тот же счётчик, а их объём от числа шагов не
    # зависит. Прежняя формула 200 + 140·n подбиралась под плоскую схему; со
    # словарём ролика ответ подорожал, и на прогоне 18 роликов 11 задач из 14
    # упали в json_validate_failed «max completion tokens reached before
    # generating a valid document». Прошли ровно те два, что уложились впритык:
    # 679 токенов на 5 шагов и 1158 на 7 при потолках 900 и 1180.
    #
